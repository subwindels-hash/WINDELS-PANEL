...
| Redis | 7.x | **optional.** Cache, sessions and job locks fall back to files; rate limiting is table-backed. Shared hosting runs without it. |
| Composer | 2.x | **development only.** Pulls `codeigniter/framework` plus the optional extras (`aws/aws-sdk-php`, `guzzle`, `predis`, `ramsey/uuid`, …). The cPanel package bundles CodeIgniter and needs none of them at runtime. |
...
There are two supported ways to run this panel, and they share one source tree:

| | Shared hosting (cPanel) | Container stack (Docker) |
|---|---|---|
...
```text
Upload application-deployment.zip   (cPanel → File Manager → Extract)
        ↓
...
encryption keys intact: **[docs/cpanel-deployment.md](docs/cpanel-deployment.md)**.

### First boot with Docker (recommended for development)
...
The release packaging pipeline is **`docs/github-actions/deployment-package.yml`**.
Paste it into **`.github/workflows/deployment-package.yml`** via the GitHub web
UI — automation tokens without the `workflows` permission cannot create that
path. Once copied it runs on `workflow_dispatch` and on every `v*` tag and
publishes `application-deployment.zip`. See
[docs/github-actions/README.md](docs/github-actions/README.md).

The broader CI suite is staged at the repo root as
**`ci.yml.workflow-ready`** (two jobs, ~31 steps, summarised below). To
activate it, copy it to `.github/workflows/ci.yml`.
...
docs/github-actions/deployment-package.yml
                      packaging workflow — paste into .github/workflows/ to activate
ci.yml.workflow-ready staged full CI suite (copy to .github/workflows/ci.yml to activate)
...
| `docs/github-actions/deployment-package.yml` | packaging workflow YAML — paste into `.github/workflows/` to activate (automation cannot write that path) |
