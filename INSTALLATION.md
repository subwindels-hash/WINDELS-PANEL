...
| Page loads but only the announcement bar shows — empty body, tab keeps working | The HTML after that strip is being parsed as script text, which happens when an inline `<script …>` start tag is missing its closing `>`. It is a markup fault, not a server fault: `/health/live` returns `{"status":"ok"}` and `deploy-verify.php` reports READY while the page is blank. Re-upload `application-deployment.zip` (or at minimum `application/views/partials/announcement_bar.php`) from a current build. |
