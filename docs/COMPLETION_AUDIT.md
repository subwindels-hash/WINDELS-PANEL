...
- **PASS + CI** — code verified; final proof requires native GitHub Actions pass (now unblocked by `.github/workflows/ci.yml` activation).
...
5. CI workflow activation — `ci.yml.workflow-ready` not moved; **fixed in cfc1b36** (`.github/workflows/ci.yml` now active).
