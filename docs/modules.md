...
| 37 | Wallets that hold a foreign currency | `wallets.currency` said NGN on every row because nothing could set it to anything else and nothing that charged a wallet knew what to do if it had been — a diaspora customer could *see* dollar prices but never hold a dollar, and the unfinished ledger said plainly what was missing: conversion at the ledger boundary and a refund-rate policy. | [module-multi-currency-wallets.md](module-multi-currency-wallets.md) |
