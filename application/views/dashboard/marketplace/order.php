...
?>
<div class="row justify-between mb-4"><a class="btn btn-ghost btn-sm" href="<?=site_url('dashboard/marketplace/orders'.($is_seller ? '?as=SELLER' : ''))?>">← Orders</a><span class="badge badge-default"><?=htmlspecialchars($order->status)?></span></div>
...
<aside>
  <?php if ($is_seller && $order->status === 'PAID'): ?><div class="card mb-4"><h3 class="card-title">Submit delivery</h3><p class="text-xs muted">Codes, credentials and instructions are encrypted before storage.</p><form method="post" action="<?=site_url('dashboard/marketplace/orders/'.$order->public_id.'/deliver')?>"><?=$csrf?><textarea class="textarea mb-3" name="delivery" minlength="3" maxlength="20000" rows="8" required></textarea><button class="btn btn-primary" type="submit">Deliver securely</button></form></div><?php endif; ?>
  <?php if ($is_buyer && $order->status === 'DELIVERED'): ?><div class="card mb-4"><h3 class="card-title">Review delivery</h3><form method="post" action="<?=site_url('dashboard/marketplace/orders/'.$order->public_id.'/accept')?>" class="mb-3"><?=$csrf?><button class="btn btn-primary" type="submit">Accept and pay seller</button></form><form method="post" action="<?=site_url('dashboard/marketplace/orders/'.$order->public_id.'/dispute')?>"><?=$csrf?><label class="label">Problem with delivery</label><textarea class="textarea mb-2" name="reason" minlength="10" maxlength="1000" required></textarea><button class="btn btn-secondary" type="submit">Open dispute</button></form></div><?php endif; ?>
  <div class="card"><h3 class="card-title">Escrow</h3><?php if ($order->status === 'COMPLETED'): ?><p class="text-sm">Seller paid <?=windels_money($order->seller_amount)?>. Platform fee <?=windels_money($order->fee_amount)?>.</p><?php elseif ($order->status === 'REFUNDED'): ?><p class="text-sm">The buyer's <?=windels_money($order->gross_amount)?> payment was refunded.</p><?php else: ?><p class="text-sm muted">Payment of <?=windels_money($order->gross_amount)?> is secured until this order is resolved.</p><?php endif; ?></div>
</aside>
</div>
