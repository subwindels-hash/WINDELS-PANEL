...
<aside>
  ...
  <div class="card"><h3 class="card-title">Escrow</h3><?php if ($order->status === 'COMPLETED'): ?><p class="text-sm">Payment of <?=windels_money($order->gross_amount)?> was released to the platform<?=$order->fee_amount ? ' (fee '.windels_money($order->fee_amount).')' : ''?>.</p><?php elseif ($order->status === 'REFUNDED'): ?><p class="text-sm">Your <?=windels_money($order->gross_amount)?> payment was refunded to your wallet.</p><?php else: ?><p class="text-sm muted">Payment of <?=windels_money($order->gross_amount)?> is secured until this order is resolved.</p><?php endif; ?></div>
</aside>
</div>
