...
<div>
  <div class="card mb-4">
    ...
    <dl class="text-sm" style="display:grid;grid-template-columns:10rem 1fr;gap:.6rem"><dt class="muted">Order</dt><dd class="mono"><?=htmlspecialchars($order->public_id)?></dd><dt class="muted">Buyer</dt><dd><?=htmlspecialchars((string)$order->buyer_username)?></dd><dt class="muted">Quantity</dt><dd><?=(int)$order->quantity?></dd><dt class="muted">Unit price</dt><dd><?=marvy_money($order->unit_price, $order->currency ?? marvy_base_currency())?></dd><?php if (bccomp((string)($order->shipping_cost ?? '0'), '0', 8) > 0): ?><dt class="muted">Shipping</dt><dd><?=marvy_money($order->shipping_cost, $order->currency ?? marvy_base_currency())?></dd><?php endif; ?><dt class="muted">Gross</dt><dd><?=marvy_money($order->gross_amount, $order->currency ?? marvy_base_currency())?></dd><dt class="muted">Ordered</dt><dd><?=htmlspecialchars($order->created_at)?></dd><?php if ($order->delivered_at): ?><dt class="muted">Delivered</dt><dd><?=htmlspecialchars($order->delivered_at)?></dd><?php endif; ?><?php if ($order->release_due_at): ?><dt class="muted">Auto-release</dt><dd><?=htmlspecialchars($order->release_due_at)?> UTC</dd><?php endif; ?></dl>
  </div>
...
<aside>
  ...
  <div class="card"><h3 class="card-title">Escrow</h3><?php if ($order->status === 'COMPLETED'): ?><p class="text-sm">Payment of <?=marvy_money($order->gross_amount, $order->currency ?? marvy_base_currency())?> was settled as platform revenue.</p><?php elseif ($order->status === 'REFUNDED'): ?><p class="text-sm">Your <?=marvy_money($order->gross_amount, $order->currency ?? marvy_base_currency())?> payment was refunded to your wallet.</p><?php else: ?><p class="text-sm muted">Payment of <?=marvy_money($order->gross_amount, $order->currency ?? marvy_base_currency())?> is secured until this order is resolved.</p><?php endif; ?></div>
</aside>
</div>
