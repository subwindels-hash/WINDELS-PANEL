...
<div class="row justify-between mb-4" style="align-items:flex-start;flex-wrap:wrap;gap:.75rem">
  <div>
    ...
    <p class="muted text-sm"><?=number_format((int)$total)?> subscription<?=$total == 1 ? '' : 's'?> matching this view</p>
  </div>
  <form method="get" action="<?=site_url($base_url)?>" class="row" style="gap:.35rem">
    <input class="input" name="q" value="<?=htmlspecialchars((string)($filters['search'] ?? ''))?>"
           placeholder="Subscription reference or target" aria-label="Search subscriptions" style="min-width:15rem">
...
<div class="card">
  <?php if (empty($rows)): ?>
    <p class="muted">No subscriptions match this filter.</p>
  <?php else: ?>
  <div class="overflow-x-auto">
    <table class="table">
      <thead>
        <tr><th>Subscription</th><th>Customer</th><th>Service</th><th>Progress</th>
            <th class="text-xs">Interval</th><th>Next run</th><th>Status</th><th></th></tr>
      </thead>
      <tbody>
      <?php foreach ($rows as $r): ?>
        ...
        <tr>
          <td>
            ...
            <div class="text-xs muted"><?=htmlspecialchars(mb_strimwidth((string)$r->target, 0, 40, '…'))?></div>
          </td>
          <td>
          ...
          <td class="text-xs muted"><?=htmlspecialchars((string)$r->interval_type)?></td>
          <td class="text-xs muted whitespace-nowrap">
            <?=$r->next_execution_at ? htmlspecialchars(date('M j, H:i', strtotime($r->next_execution_at))) : '—'?>
          </td>
